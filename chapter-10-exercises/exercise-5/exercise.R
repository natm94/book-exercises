...
names <- read.csv("data/female_names.csv", stringsAsFactor = F)
...
most_popular_name <- names_2013$name[names_2013$prop == max(names_2013$prop)]

# Write a function `most_popular_in_year` that takes in a year as a value and 
# returns the most popular name in that year

most_popular_in_a_year <- function(year_value) {
  year_subset <- subset(names, names$year == year_value)
  year_poluplar <- year_subset[year_subset$prop == max(year_subset$prop),]
  year_poluplar["name"]
}
...
number_in_a_million <- function(name, year)
